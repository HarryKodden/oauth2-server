	...
	for _, client := range clients {
		if storeClient, ok := client.(*store.Client); ok {
			clientInfo := map[string]interface{}{
				...
				"redirect_uris_resolved":     storeClient.GetResolvedRedirectURIs(r, h.config.Server.BaseURL),
...
func (h *DocsHandler) createClient(w http.ResponseWriter, r *http.Request) {
	...
	response := map[string]interface{}{
		...
		"redirect_uris_resolved":     newClient.GetResolvedRedirectURIs(r, h.config.Server.BaseURL),
